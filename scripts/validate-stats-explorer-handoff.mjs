...
assert(map.status === 'handoff_for_e5_1_query_contract', 'handoff status is incorrect')
assert(map.explorer_route === '/explore/', 'explorer route is incorrect')
assert(map.rules?.url_contract_finalized === false, 'E-5 must not finalize the Explorer URL contract')
...
console.log(`Validated Stats Explorer handoff: ${map.dimensions.length} dimensions, ${seenPaths.size} Stats paths, ${directSources.size} direct reviewed-data sources.`)
