...
const queuePath = path.join(root, 'config', 'canonical-only-active-cex-queue.json')
...
function loadQueue() {
  ...
  if (!queue.scope || queue.scope.type !== 'cex' || !Array.isArray(queue.scope.statuses)) throw new Error('queue scope must define cex statuses')
  return queue
}
...
const activeStatuses = new Set(queue.scope.statuses)
const errors = []
const ids = new Set()
...
for (const [batchNumber, entries] of [...batches.entries()].sort((left, right) => left[0] - right[0])) {
  if (entries.length !== queue.batch_size) errors.push(`batch ${batchNumber}: expected ${queue.batch_size} entries, got ${entries.length}`)
...
const currentCanonicalOnly = canonical.entities
  .filter((entity) => entity.type === queue.scope.type && activeStatuses.has(entity.status) && !bundleByEntityId.has(entity.id))
...
for (const entity of currentCanonicalOnly) {
  if (!pendingIds.has(entity.id)) errors.push(`${entity.id}: canonical-only active-side CEX is missing from pending queue`)
}
...
const markdown = [
  '# HEI Canonical-only Active CEX Queue',
  '',
  ...
  ...(report.current_batch.length > 0
    ? report.current_batch.map((entry) => `- ${entry.canonical_name} (${entry.id}) — score ${entry.initial_repair_score}`)
...
console.log(`Canonical-only active CEX queue: ${report.status}`)
