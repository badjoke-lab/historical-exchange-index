...
const authorityPath = path.join(root, 'config', 'ledger-series-phase9-stage6-v31-corrective-reverification-authority-2026-08-23.json')
const baselinePath = path.join(root, 'config', 'ledger-series-phase9-stage6-v31-execution-baseline.json')
const legacyPath = path.join(root, 'scripts', 'verify-ledger-series-phase9-stage6-production-equality.mjs')
const runtimePath = path.join(root, '.stage6', 'phase9-stage6-v31-runtime-verifier.mjs')
...
assert(authority.authority_id === 'hei-ledger-series-phase9-stage6-v31-corrective-reverification-2026-08-23', 'unexpected v3.1 authority id')
assert(authority.failed_execution?.workflow_run_id === 32614056946, 'v3.1 authority must bind to the consumed v3 failure')
assert(authority.corrective_scope?.mag_correct_expected_build === 'f917d5e25eedc7b2c48091c7343b7fa9cd203428', 'unexpected MAG corrective build identity')
assert(authorization.corrective_implementation_authorized === true, 'corrective implementation not authorized')
assert(authorization.network_read_only_corrective_reverification_authorized_after_corrective_implementation_merge === true, 'corrective network reverification not authorized')
assert(authorization.corrective_network_execution_count_authorized === 1, 'corrective network execution count must remain one')
assert(authorization.rerun_failed_v3_workflow_authorized === false, 'consumed v3 rerun must remain forbidden')
assert(authorization.production_mutation_authorized === false, 'production mutation boundary weakened')
assert(authorization.vertical_repository_mutation_authorized === false, 'vertical mutation boundary weakened')
assert(authorization.canonical_record_mutation_authorized === false, 'canonical mutation boundary weakened')
assert(authorization.relationship_mutation_authorized === false, 'relationship mutation boundary weakened')
assert(authorization.central_descriptor_resync_authorized === false, 'descriptor resync boundary weakened')
assert(authorization.cloudflare_dns_deployment_mutation_authorized === false, 'deployment mutation boundary weakened')
assert(authorization.automatic_retry_authorized === false, 'automatic retry must remain forbidden')
...
assert(byId.get('minted-and-gone').production_runtime_source === byId.get('minted-and-gone').reviewed_main, 'MAG corrected production build must equal reviewed main')
...
source = replaceOnce(
  ...
  "config', 'ledger-series-phase9-stage6-v31-corrective-reverification-authority-2026-08-23.json'",
  'authority path',
)
...
source = replaceOnce(
  ...
  "assert(authority.authority_id === 'hei-ledger-series-phase9-stage6-v31-corrective-reverification-2026-08-23', 'unexpected Stage 6 v3.1 authority')",
...
source = replaceOnce(
  ...
  "assert(authority.authorization?.network_read_only_corrective_reverification_authorized_after_corrective_implementation_merge === true, 'Stage 6 v3.1 corrective network reverification is not authorized')",
...
source = replaceOnce(
  ...
  "assert(authority.authorization?.production_mutation_authorized === false && authority.authorization?.vertical_repository_mutation_authorized === false && authority.authorization?.central_descriptor_resync_authorized === false, 'Stage 6 v3.1 mutation boundary weakened')",
...
const authorityLoopReplacement = `  const frozen = byId.get(id)
  ...
  if (id !== 'historical-exchange-index') {
    assert(frozen.reviewed_main === review.repo_main, \`${'${id}'}: reviewed main changed from corrective authority\`)
  }`
source = replaceOnce(source, authorityLoopNeedle, authorityLoopReplacement, 'corrective authority baseline loop')

source = replaceOnce(source, "schema_version: '1.1.0'", "schema_version: '1.3.0'", 'report schema version')
source = replaceOnce(
  source,
  "execution_kind: 'read_only_cross_registry_production_equality_corrected'",
  "execution_kind: 'read_only_cross_registry_current_production_corrective_reverification_v31'",
  'execution kind',
)
source = replaceOnce(
  source,
  "      assert(githubSha && observed === githubSha, `HEI main moved: workflow ${githubSha || 'missing'}, observed ${observed}`)",
  "      const expectedHeiExecutionSha = (process.env.STAGE6_HEI_EXECUTION_SHA || githubSha || '').trim()\n      assert(expectedHeiExecutionSha && observed === expectedHeiExecutionSha, `HEI main moved: corrective execution ${expectedHeiExecutionSha || 'missing'}, observed ${observed}`)",
...
source = replaceOnce(source, heiNeedle, heiReplacement, 'HEI v3.1 build verification')
...
source = replaceOnce(source, cyaNeedle, cyaReplacement, 'CYA v3.1 build/source verification')

source = replaceOnce(
  source,
  "  authority_id: authority.authority_id,",
  "  authority_id: authority.authority_id,\n  execution_baseline_id: 'hei-ledger-series-phase9-stage6-v31-execution-baseline-2026-08-23',\n  corrective_parent_run_id: 32614056946,",
  'report corrective metadata',
)
source = replaceOnce(
  source,
  "report.overall = 'PASS'",
...
if (syntax.status !== 0) {
  ...
  throw new Error(`v3.1 runtime verifier syntax check failed with ${syntax.status}`)
}
...
console.log(JSON.stringify({
  ...
  corrected_mag_build: byId.get('minted-and-gone').production_runtime_source,
